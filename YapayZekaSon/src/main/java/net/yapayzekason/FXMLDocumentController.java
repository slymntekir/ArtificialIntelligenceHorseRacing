...
public class FXMLDocumentController implements Initializable {
	...
	private void Calculate(ActionEvent event) {
		...
		int rakam = e.Resultfound(veri);
		lbl_sonuc.setText("" + rakam);
	}

	public Object nesneBul(String incoming) {
		for (int i = 0; i < root.getChildren().size(); i++) {
			String name = root.getChildren().get(i).getId();
			if (name != null)
	...
	public void dosyaOKu() throws Exception {
		...
		sequence = new double[satir][sutun];
		...
		for (int i = 0; i < satir; i++) {
			...
			for (int j = 0; j < sutun; j++) {
				if (j == 0) {
					sequence[i][j] = Double.parseDouble(text.split("-----")[j]) * 0.1;
					try {
						sequence[i][j] = Double.parseDouble(String.valueOf(sequence[i][j]).substring(0, 3));
					} catch (Exception e) {
					}
				} else if (j == 1) {
					sequence[i][j] = Double.parseDouble(text.split("-----")[j]) * 0.001;
					try {
						sequence[i][j] = Double.parseDouble(String.valueOf(sequence[i][j]).substring(0, 5));
					} catch (Exception e) {
					}
				} else if (j == 2) {
					sequence[i][j] = Double.parseDouble(text.split("-----")[j]) * 0.01;
					try {
						sequence[i][j] = Double.parseDouble(String.valueOf(sequence[i][j]).substring(0, 4));
					} catch (Exception e) {
					}
				} else if (j == 3) {
					sequence[i][j] = Double.parseDouble(text.split("-----")[j]) * 0.01;
		...
		for (int a = 0; a < 100; a++) {
			System.out.println(sequence[a][0] + "  " + sequence[a][1] + "  " + sequence[a][2] + "  " + sequence[a][3] + "  " + sequence[a][4]);
		}
